...
my_function <- function(action, lattice, backfill_option, p_target, p_target_efficacy, ppat_selection, backfill_strategy, CPAT_efficacy_threshold_for_backfill, efficacy_trade_off, n_stop_total, n_stop_BOLD_1, n_stop_BOLD, prior_information, dlt_prior_input_1_2, dlt_prior_input_1_3, dlt_prior_input_1_4, dlt_prior_input_1_5, dlt_prior_input_2_2, cur_b_1_2, cur_b_1_3, cur_b_1_4, cur_b_1_5, cur_a_2_2, cur_b_2_2, n_p, n_input_MTD_1_2, n_input_MTD_1_3, n_input_MTD_1_4, n_input_MTD_1_5, n_input_MTD_2_2, n_input_Backfill_1_2, n_input_Backfill_1_3, n_input_Backfill_1_4, n_input_Backfill_1_5, n_input_Backfill_2_2, x_toxic_input_1_2, x_toxic_input_1_3, x_toxic_input_1_4, x_toxic_input_1_5, x_toxic_input_2_2,  x_efficacy_input_1_2, x_efficacy_input_1_3, x_efficacy_input_1_4, x_efficacy_input_1_5, x_efficacy_input_2_2, pess_input_1_2, pess_input_1_3, pess_input_1_4, pess_input_1_5, pess_input_2_2, threshold_input_1_2_non_informative, threshold_input_1_3_non_informative, threshold_input_1_4_non_informative, threshold_input_1_5_non_informative, threshold_input_2_2_non_informative, threshold_input_1_2_informative, threshold_input_1_3_informative, threshold_input_1_4_informative, threshold_input_1_5_informative, threshold_input_2_2_informative, stage, dlt_true_input_1_2, dlt_true_input_1_3, dlt_true_input_1_4, dlt_true_input_1_5, dlt_true_input_2_2, efficacy_true_input_1_2, efficacy_true_input_1_3, efficacy_true_input_1_4, efficacy_true_input_1_5, efficacy_true_input_2_2, simulation_runs)
   ...
   { #2
    for (j in 1:level_b)
     { #3
      ...
      if (backfill==1)
       { #4
        a[[i]]$prior_alpha_efficacy[j] <- 1 # Assuming uniform Beta(1,1)
        a[[i]]$prior_beta_efficacy[j] <- 1 
       } #4
     } #3
   } #2
 ...
 { #5
  ...
  if (stopping==1 & backfill==1 & level_a==1) # Identify OBD only if any criterion of general stopping rules is met and backfilling was applied.
    { #4
      final_OBD_b <- 0 # Default OBD (No OBD is identified)
      BOLD_mean_matrix_efficacy_pava_local <- matrix(100, nrow=level_a, ncol=level_b) # The default posterior mean efficacy rate after local PAVA for OBD identification.
     if (sum(BOLD_mtd_contour_matrix)>0) # There is a MTD identification
       { #5
        for (j in 1:BOLD_final_MTD_b) # Calculate the local PAVA posterior mean efficacy for each dose level not exceeding the identified MTD.
          { #6
            if (j==1) # There is no anti-cover for dose 1.
              { #7
                BOLD_mean_matrix_efficacy_pava_local[1,j] <- pava(BOLD_mean_matrix_efficacy[1,c(j,j+1)], a[[1]]$n_accumulated[c(j,j+1)])[1] # Apply local PAVA
              } #7
           if (j==level_b) # There is no cover dose for the highest dose
              { #7
                BOLD_mean_matrix_efficacy_pava_local[1,j] <- pava(BOLD_mean_matrix_efficacy[1,c(j-1,j)], a[[1]]$n_accumulated[c(j-1,j)])[2] # Apply local PAVA
              } #7
            if (j>1 & j<level_b) # There are both cover and anti-cover doses.
              { #7
                BOLD_mean_matrix_efficacy_pava_local[1,j] <- pava(BOLD_mean_matrix_efficacy[1,c(j-1,j,j+1)], a[[1]]$n_accumulated[c(j-1,j,j+1)])[2] # Apply local PAVA
              } #7
           } #6
        for (j in 1:BOLD_final_MTD_b) # Filter for trade-off and effectiveness.
          { #6
           if (BOLD_mean_matrix_efficacy_pava_local[1,j] < (1-efficacy_trade_off)*BOLD_mean_matrix_efficacy_pava_local[1,BOLD_final_MTD_b] | BOLD_mean_matrix_efficacy_pava_local[1,j] < p_target_efficacy) 
            { #7
              BOLD_mean_matrix_efficacy_pava_local[1,j] <- 100
            } #7
          } #6
       if (sum(BOLD_mean_matrix_efficacy_pava_local) < 100*level_a*level_b) # There is at least one OBD candidate.
          { #6
            j <- 1 # Initial dose for OBD identification process.
            repeat
             { #7
              if (BOLD_mean_matrix_efficacy_pava_local[1,j]<100)
                { #8
                 final_OBD_b <- j
                 BOLD_obd_contour_matrix[1,final_OBD_b] <- 1        
                 break
                } else #8
                { #8
                  j <- j+1 # Increase dose level for OBD identification process.
                } #8
             } #7     
           } #6
       } #5
...
ui <- bslib::page_sidebar(
  ...
  conditionalPanel(
      condition = c("input.backfill_option == 'Yes'"), sliderInput("CPAT_efficacy_threshold_for_backfill", label="What is the CPAT activity threshold for a dose to qualify as a backfilling candidate? (Default value is 0.2)", min=0.05, max=0.4, value=0.2, step=0.01)), # Specify the CPAT activity threshold for a dose to qualify as a backfilling candidate.
  conditionalPanel(
...
matrixInput(
  ...
  label="Please input the non-descending true activity rates for simulation:",
  value = matrix(c(0.35,0.5), 1, 2, dimnames = list("A1", c("B1", "B2"))),   rows = list(names = TRUE),
  cols = list(names = TRUE)
)),
    conditionalPanel(
...
matrixInput(
  ...
  label="Please input the non-descending true activity rates for simulation:",
  value = matrix(c(0.35,0.5,0.5), 1, 3, dimnames = list("A1", c("B1", "B2", "B3"))),
  rows = list(names = TRUE),
  cols = list(names = TRUE)
)),
 conditionalPanel(
...
matrixInput(
  ...
  label="Please input the non-descending true activity rates for simulation:",
  value = matrix(c(0.25,0.35,0.5,0.5), 1, 4, dimnames = list("A1", c("B1", "B2", "B3", "B4"))),
  rows = list(names = TRUE),
  cols = list(names = TRUE)
)),
 conditionalPanel(
...
matrixInput(
  ...
  label="Please input the non-descending true activity rates for simulation:",
  value = matrix(c(0.25,0.35,0.5,0.5,0.5), 1, 5, dimnames = list("A1", c("B1", "B2", "B3", "B4", "B5"))),
  rows = list(names = TRUE),
  cols = list(names = TRUE)
)),
 conditionalPanel(
